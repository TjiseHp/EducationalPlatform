...
<script type="text/javascript">
		...
		function doDel(uId) {
			layer.confirm('确认删除？', {
				...
				window.location.href = "${pageContext.request.contextPath}/pay/deletePay?uId="+uId;
			}, function(){
		...
		function doUpdate(uId) {		
			window.location.href = "${pageContext.request.contextPath}/pay/updatePay?uId="+uId;
		}
		
		function dodetail(uId){
			window.location.href = "${pageContext.request.contextPath}/pay/payDetail?uId="+uId;
		}
	
		function doSearch(){
			var search = $("#search").val();
			if(search == ""){
				window.location.href = "${pageContext.request.contextPath}/pay/payTable";
			}else{
				window.location.href = "${pageContext.request.contextPath}/pay/searchPay?search="+search;
			}
		}
				  		...
				  		<form action="">
				  			...
				  			<div>
					  			...
					  			<tr class="demo-table-hover">
					  				...
					  				<td>
					  					<a class="btn btn-success btn-xs" role="button" href="javascript:;" onclick="doAdd();" >新增</a>
					  				</td>
					  			</tr>
					  			<c:forEach items="${pageInfo.list}" var="pays">
					  				<tr>
						  				<td>${pays.payNum}</td>
						  				<td>${pays.uId}</td>
						  				<td>${pays.payText}</td>
