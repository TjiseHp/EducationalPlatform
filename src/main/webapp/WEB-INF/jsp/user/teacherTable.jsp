...
<script type="text/javascript">
	function doDel(uId) {
		layer
				.confirm(
						...
						function() {
							window.location.href = "${pageContext.request.contextPath}/user/deleteTeacher?uId="
									+ uId;
						}, function() {
	...
	function doUpdate(uId) {
		window.location.href = "${pageContext.request.contextPath}/user/updateTeacher?uId="
				+ uId;
	}
	...
	function doSearch() {
		...
		if (search == "") {
			...
			window.location.href = "${pageContext.request.contextPath}/user/searchTeacher?search="
					+ search;
		}
	}
				...
				<c:forEach items="${pageInfo.list}" var="teacher">
					<tr>
						...
						<td><a class="btn btn-primary btn-xs" role="button"
							href="javascript:;" onclick="doUpdate('${teacher.uId}');">修改</a>
							<a class="btn btn-danger btn-xs" role="button"
							href="javascript:;" onclick="doDel('${teacher.uId}');">删除</a></td>
					</tr>
				</c:forEach>
			</table>
		</div>
...
<div class="text-center">
	...
	<c:if test="${pageInfo.hasPreviousPage}">
		<a class="btn btn-info"
			href="teacherTable?pageNum=${pageInfo.pageNum-1}">上一页</a>
	</c:if>
	<c:if test="${pageInfo.hasNextPage}">
		<a class="btn btn-info"
			href="teacherTable?pageNum=${pageInfo.pageNum+1}">下一页</a>
	</c:if>
